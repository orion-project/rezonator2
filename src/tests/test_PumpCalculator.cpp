...
TEST_METHOD(Waist_gauss)
{
    ...
    ASSERT_IS_TRUE(calc->init(&p, 980e-9));

    // Beam at schema entrance
    BeamResult beam = calc->calc(Matrix());
    ...
    // Output beam
    beam = calc->calc({ -1.4379022, 0.1681906, -10.1543000, 0.4922850 });
...
TEST_METHOD(Waist_hyper)
{
    ...
    // Output beam
    beam = calc->calc({ -1.3899498, 0.1731843, -9.8480800, 0.5075960 });
